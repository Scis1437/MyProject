        ...
        <table className="table-auto w-full">
          ...
          <tbody>
            {Object.entries(title).map(([id, title]) => (
              <tr key={id} className="bg-gray-100 border-b mx-4 odd:bg-white even:bg-slate-50 cursor-pointer">
                <td className="py-4 whitespace-nowrap text-sm font-medium text-gray-900">{title}</td>
                <td className="text-sm text-gray-900 font-light px-6 py-4 whitespace-nowrap text-end">
                  <select onChange={(e) => handlePointChange(id, e.target.value)}>
                    <option value=""></option>
